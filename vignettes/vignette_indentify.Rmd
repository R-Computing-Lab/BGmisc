...
Let us fit the mz only data

```{r}

run1 <- emxTwinModel(model='Cholesky', relatedness='RCoef',
               data=mzData, use=selVars, run=TRUE, name='TwCh')

```

As you can see the model was unsucessful because it was not identified. But when we add another group, so that the model is identified.

```{r}
run2 <- emxTwinModel(model='Cholesky', relatedness='RCoef',
