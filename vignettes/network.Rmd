...
plotPedigree(hazard, code_male = 0, verbose = TRUE,)
```
...
Because the `hazard` data already had a family ID variable we compare
...
individuals in the `hazard` dataset.
...
the `hazard` dataset through sharing both parents.
...
Here we calculate the relatedness between all pairs of individuals in
the `hazard` dataset through sharing an extended family.
