...
We will use the `potter` pedigree data as an example. For convienence, we've renamed the family ID variable to `oldfam` to avoid confusion with the new family ID variable we will create.

```{r}
names(potter)[names(potter)=="famID"]<- "oldfam"

ds <- ped2fam(potter, famID = "oldfam", personID = "personID")

table(ds$famID, ds$fam)
```
...
Subsets of pedigrees can also be plotted to zoom in on smaller portions of pedigrees.
However, this subset does not plot the relationship between spouses (such as the marriage between Vernon (1) and Petunia (3)), as there are not children to connect the two individuals together yet
