# temporal_BGmisc_ACE_parameter_recovery.R
#
...
library(snowfall)
library(parallel)
detect_cores <- parallel::detectCores(logical = FALSE) -  1L # leave one core free for the OS
# Clear any cluster left running by an earlier run that errored before sfStop().
# Without this, the next sfInit reuses dead sockets -> "invalid connection".
if (sfIsRunning()) try(sfStop(), silent = TRUE)
sfInit(parallel = TRUE, cpus = detect_cores)
sfLibrary(OpenMx)
...
build_true_model <- function(families, replication) {
  ...
  temporal_model_ace <- free_only(
    temporal_model_ace,
    labels_to_free = labels_to_free
  )

  mxOption(temporal_model_ace, "Number of Threads", omx_threads)
}
...
# Each snowfall worker is a fresh R session that knows nothing about this script.
# Give it: the packages it needs, the helper functions (sourced by absolute path,
# since PSOCK workers do not inherit the working directory), and every setting
# and function defined above (sfExportAll ships the current global environment).
# run_one_replication reseeds deterministically from master_seed + replication, so
# results are identical whether run in parallel or sequentially and no parallel-RNG
# setup is needed.
sfLibrary(BGmisc)
sfLibrary(mvtnorm)
sfLibrary(dplyr)
...
sfLibrary(tibble)
sfLibrary(readr)
sfSource(normalizePath(file.path("data-raw", "smoketest_helpers.R")))
sfExportAll()

replication_results <- vector("list", n_replications)

# Run replications across workers in checkpoint-sized batches. Every worker stays
# busy within a batch, and results are written after each batch, so a crash costs
# at most one batch (save_rate replications) rather than the whole run.
batches <- split(
  seq_len(n_replications),
  ceiling(seq_len(n_replications) / save_rate)
)

for (batch in batches) {
  cat(sprintf(
    "Running replications %d-%d of %d\n",
    min(batch), max(batch), n_replications
  ))

  replication_results[batch] <- sfLapply(batch, run_one_replication)

  if (save_checkpoints) {
    done <- which(!vapply(replication_results, is.null, logical(1)))
    readr::write_csv(
      dplyr::bind_rows(replication_results[done]),
      replication_file,
      na = ""
    )
  }
...
FIGURE <- TRUE
if (FIGURE) {
  source(file.path("data-raw", "temporal_basic_mc_plots.R"))
}

# notes Parameter recovery: To preliminarily evaluate our ability to estimate the parameters in Equation 1, we ran a small Monte Carlo study, generating 1410 pedigrees each of average size (N=44 individuals/pedigree) with different kinds of relationships. The total N in each run was thus 62,040, at the lower end of our likely sample size. We fitted the true model to this sample using the regression method detailed above, generating data and fitting the model 500 times. Our true parameter values were: \mathbf{a}^\mathbf{2} = 40%, \mathbf{d}^\mathbf{2} = 0% (estimable, but fixed for this demonstration), \mathbf{c}_\mathbf{N}^\mathbf{2} = 15%, \mathbf{c}_\mathbf{E}^\mathbf{2}= 6%, \mathbf{m}\mathbf{t}^\mathbf{2} = 3%, \mathbf{j}^\mathbf{2} = 1%, \mathbf{e}^\mathbf{2} = 35%. As shown in the final box in Fig. 5, mean parameter estimates from our model were virtually identical to the true values. For all parameters, the Monte Carlo mean parameter estimate was almost identical to the true generating value (e.g., root mean squared error [RMSE] < 0.001), and the Monte Carlo 95% CI’s indicate power was sufficient to distinguish the non-zero parameters from zero. That said, further work is needed to fully establish the quality of parameter estimates. A large Monte Carlo study of 50,000 pedigrees of different sizes and structures will be conducted as part of our model validation procedures. If the developed model does not recover parameters as well as seen in this pilot exploration, the model will be adjusted until it sufficiently recovers all parameters.
