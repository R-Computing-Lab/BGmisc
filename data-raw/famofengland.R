...
#---
# source Families of England data
#--
families_of_england <- read.csv("E:/Dropbox/Lab/Research/Projects/2024/FamiliesofEngland/data/pnas_clean.csv")

foe_skinny <- families_of_england %>%
  dplyr::select(ID, dadID, momID, sex =sex_factor, birthYear= byr, age) %>%
...
for (i in seq_along(usable_fams)) {
  fam_i <- subset(ped_foe, famID == usable_fams[i])
...
# Build family group models for temporal A + E.
  group_models[[i]] <- buildOneTemporalFamilyGroup(
    ...
    full_df_row = fam_i$age,
    obs_ids = fam_i$personID,
    birth_year = fam_i$birthYear,
    H = fam_i %>% mutate(post_1776 = ifelse(birthYear >= 1776, 1, 0)) %>% select(post_1776) %>% as.matrix(),
