...
scopes <- list(
  ...
  conv = c("component", "twin_method", "beta", "sparse_matrix", "transpose_method")
)
...
write_csv(design, "data-raw/ped2com_benchmark_design_com.csv")
# write start time
...
summary(results)
results %>%
  group_by(ped_label, transpose_method, twin_method, sparse_matrix) %>%
  summarise(
