...
test_that("sliceFamilies creates correct output files for both mitRel = 1 and 0", {
  ...
  test_data <- data.frame(
    ID1 = c(1, 2, 3, 4),
    ID2 = c(5, 6, 7, 8),
    addRel = c(0.25, 0.125, 0.0625, 0.03125),
    mitRel = c(1, 0, 1, 0),
    cnuRel = c(1, 1, 0, 0)
  )
  input_file <- file.path(tmp, "test_input.csv")
  fwrite(test_data, input_file)
  ...
  sliceFamilies(
    ...
    bin_width = 0.10,
    degreerelatedness = 2,
    chunk_size = 3,  # small chunk size to force chunk logic
    max_lines = 100,
