...
appears anywhere in each person's ordered unilineal parental ancestor
chain.
...
- component:
  ...
  Character string specifying which parental component the chain
  represents. Must be either \`"dadID"\` for paternal chains or
  \`"momID"\` for maternal chains.

## Value

A data frame with \`flag_col\` added. The flag is \`TRUE\` when
\`anchor_id\` appears in the individual's selected parental chain and
\`FALSE\` otherwise.
...
The anchor ID is coerced to character before comparison because the
chain columns produced by \[addParentalChain()\] store graph vertex
names as character values.

This function assumes that \`chain_col\` is a list-column in which each
element is a character vector of ancestor IDs. Empty chains should be
represented as \`character(0)\`.
